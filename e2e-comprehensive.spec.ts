...
// Standard localStorage payload that bypasses onboarding, dialect selection, and placement test
const BASE_STORAGE = {
    onboarding_completed: 'true',
    selectedDialect: 'msa',
    godMode: 'false',
    arabic_app_progress: JSON.stringify({ dummy_node: { score: 100 } }),
};

// Helper: set localStorage values on a page that is already at the app origin
async function setStorage(page: Page, extras: Record<string, string> = {}) {
    const items = { ...BASE_STORAGE, ...extras };
...
// Helper to unlock the app and land on the learn grid
async function unlockApp(page: Page) {
    // 1. Visit root so we have the correct origin for localStorage
    await page.goto('http://localhost:5173/');
    // 2. Inject all required flags BEFORE navigating to the guarded route
    await setStorage(page);
    // 3. Navigate – React will mount and see the correct localStorage immediately
    await page.goto('http://localhost:5173/learn/reading');
    await page.waitForSelector('.learn-page, .node', { timeout: 15000 });
}

// Helper to enable God Mode for testing (call AFTER unlockApp)
async function enableGodMode(page: Page) {
    await page.evaluate(() => localStorage.setItem('godMode', 'true'));
    // No need to reload – godMode is read per-render from localStorage
}
...
test.describe('Comprehensive Arabic Learning App Tests', () => {
    ...
    test.beforeEach(async ({ page }) => {
        // Visit root to bind origin, then inject all bypass flags
        await page.goto('http://localhost:5173/');
        await setStorage(page);
    });
    ...
    test('Knowledge Gap Check: Verify no untaught letters appear in exercises', async ({ page }) => {
        test.slow(); // This test walks an entire round tracking every letter
        await unlockApp(page);
        await enableGodMode(page);
    ...
    test('Audio Verification: All audio exercises play correctly', async ({ page }) => {
        ...
        // Audio autoplay may be blocked in some browsers (webkit, mobile) — soft pass
        if (audioExerciseCount === 0) {
            console.log('Note: Audio autoplay likely blocked in this browser — skipping assertion');
        } else {
    ...
    test('Exercise Types: Verify all exercise types render correctly', async ({ page }) => {
        ...
        // Test up to 2 nodes × 2 rounds to stay within timeout budget
        for (let nodeIndex = 0; nodeIndex < 2; nodeIndex++) {
            ...
            for (let roundIndex = 0; roundIndex < 2; roundIndex++) {
                ...
                // Stop early once we have enough types
                if (exerciseTypesFound.size >= 2) break;
            }
    ...
    test('Complete Lesson Flow: Full Round 1 with correct answers', async ({ page }) => {
        test.slow(); // This test navigates through an entire round — needs extra time
        await unlockApp(page);
        await enableGodMode(page);
    ...
    test('Trap Exercises: Verify trap explanations are shown', async ({ page }) => {
        test.slow(); // Trap exercises may require many rounds to appear
        await unlockApp(page);
        await enableGodMode(page);
        ...
        for (let roundIndex = 2; roundIndex < 5 && !foundTrapExercise; roundIndex++) {
            ...
            // Dismiss trap explanation overlay if visible
            const trapOverlay = page.locator('.trap-explanation-overlay');
            if (await trapOverlay.isVisible()) {
                const dismissBtn = page.locator('.trap-continue-btn');
    ...
    test('Multi-Node Journey: Test progression through multiple nodes', async ({ page }) => {
        test.slow(); // This test navigates through multiple nodes sequentially
        await unlockApp(page);
        await enableGodMode(page);
